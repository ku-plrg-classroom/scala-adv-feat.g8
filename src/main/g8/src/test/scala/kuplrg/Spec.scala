...
class Spec extends SpecBase {
  ...
  // ---------------------------------------------------------------------------
  {
    ...
    testExc(x41, "42", weight = 2)
    lazy val x42 = g(not[B \/ I])._1(true)
    testExc(x42, "true", weight = 3)
    lazy val x43 = f(not[S], not[B])(Right(false))
    testExc(x43, "false", weight = 2)
    lazy val x44 = g(not[I \/ S])._2("abc")
    testExc(x44, "Right(abc)", weight = 3)
  }
